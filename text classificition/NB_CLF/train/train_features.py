...
class Train:
    def __init__(self,features_dim=10000,flag='sklearn'):
    ...
    def features(self,text):
        ...
        if self.flag == 'nltk':
            ## nltk特征 dict
            features = {word: 1 if word in text_words else 0 for word in self.feature_words}
        elif self.flag == 'sklearn':
            ## sklearn特征 list
            features = [1 if word in text_words else 0 for word in self.feature_words]
        else:
            features = []
            ## -----------------------------------------------------------------------------------
        return features

    def text_features(self,path):
...
if __name__ == '__main__':
    ...
    t.text_features(setting.TRAIN_FEATURES)